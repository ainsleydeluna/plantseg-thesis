...
6. **No commits/pushes until after `git diff` + `git status` verification**; then commit with an explicit message, and push only if **the user** has approved the push — see **What counts as approval**. **[UPDATED 2026-09-23 — L-PROT P1]** Both checks name explicit paths or end with `-- . ':(exclude)docs/reference/reference.pdf'`, and commits use `git commit --only -- <declared paths>`; the committed `.claude/settings.json` hook enforces this.
