...
**B. Files inspected** — the read-only set. For the protected path, cite the ~~`git status --porcelain -- docs/reference/`~~ `git status --porcelain=v1 -- docs/reference/reference.pdf` **[UPDATED 2026-09-23 — L-PROT P1]** output pasted in section A; it must show no staged entry, and must be unchanged if re-run at the end of the pass. That `reference.pdf` was not **opened** cannot be shown by any command — state it as an **attestation** and label it one.
