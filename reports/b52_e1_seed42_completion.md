...
> **[UPDATED 2026-09-14 — B56]** The register now also carries the items raised in
> [B55](b55_teacher_acquisition.md) §9 (G12–G14, N22, N23, and N24, reclassified as G18) and in
> [B56](b56_teacher_survey.md) §8 (G15–G17, N25), so it is once again the one place to read the queue.
> Status changes are marked inline with the section they come from.
>
> **[UPDATED 2026-09-15 — B58]** G19 and G20 are added, and G8, G14, G15 and G18 carry inline updates
> from [B58](b58_teacher_runner_adjudication.md). No item is renumbered, and earlier status marks are kept.
>
> **[UPDATED 2026-09-20 — G18 CPU verification]** Reconciled against HEAD `0bb6996`. Every commit
> named here postdates the B58 text above, which is why the rows below carried stale "plan pending"
> and "to ship" marks: G16 and G17 landed in `51697a7`, G8 in `48bccc7`, and G18's implementation in
> `0bb6996`. `scripts/smoke_teacher_runner.py` was added and verifies the G18 seam on CPU (48/48,
> inside `plantseg-teacher:local`). Three statuses are deliberately distinct and must not be
> collapsed: **G18 implementation landed** · **G18 CPU verification passed** · **CUDA verification
> (G20) still open**. Nothing here closes G20, G15's operational confirmation, checkpoint readiness,
> or G12's push. No item is renumbered, and earlier status marks are kept.
>
> **[UPDATED 2026-09-20 — G12 published; N22 documented]** G12 is **CLOSED**: the already-validated
> teacher image was tagged and pushed to GHCR with no rebuild, and the published object was verified
> anonymously end to end. N22 is **CLOSED**: its invariant landed in
> [docs/runpod_environment.md](../docs/runpod_environment.md) §4b item 3 and its operator procedure in
> [docs/teacher_prep_runbook.md](../docs/teacher_prep_runbook.md) §4a. Still open and untouched by
> either: **G20** (first-forward CUDA attestation), **G15**'s operational confirmation, **checkpoint
> readiness**, and G2/P-5B. The G20 runtime checkout stays **`0bb6996`** — `7576ba8` adds CPU
> verification and reporting only, and the three runtime-critical files are byte-identical across the
> two commits.
...
| G8 *(raised as N18)* | Set `CUBLAS_WORKSPACE_CONFIG=:4096:8` as a Dockerfile `ENV` so the value is attested by the image, not only by source ordering — **default is DON'T**, see §11.3. **[UPDATED 2026-09-14 — B56 §3.5] NECESSARY for the teacher image.** G14 measured that nothing on the teacher path sets the variable, and no teacher launcher exists yet (G18), so an `ENV` in `Dockerfile.teacher` is the only mechanism at this HEAD. The DON'T default stands for the E1 image, which seeds 2 and 3 must run against unchanged. **[UPDATED 2026-09-15 — B58] PRIMARY and NECESSARY for the teacher path — not defence in depth — and still TO SHIP:** `Dockerfile.teacher` has no `ENV` for the variable at this HEAD (B57 E-2). The selected G18 `TeacherRunner` asserts the Python-visible value before CUDA initialization and never creates it. The evidentiary roles differ: the image `ENV` declaration and digest attest what the image supplies; the launcher-entry echo and G18's first-call assertion attest what the run's own process saw. None of them inspects cuBLAS's internal workspace state (B56 §3.5; B58 §9.1, §10–11). **[CLOSED 2026-09-20 — landed in `48bccc7`]** `Dockerfile.teacher:52` declares `ENV CUBLAS_WORKSPACE_CONFIG=:4096:8`, and the built image carries it: `docker image inspect plantseg-teacher:local` reports `CUBLAS_WORKSPACE_CONFIG=:4096:8` under revision label `48bccc70caef…4368f5`. The G18 smoke was run with **no `docker run -e`**, so the value its first-call assertion accepted was supplied by the image and not by the invocation | `Dockerfile` (no `ENTRYPOINT`; `ENV` at `:79`/`:97`); `Dockerfile.teacher:52`; B56 §3.5; B58 §9.1, §10–11 |
...
| G12 | Add `ftfy==6.3.0` and build a teacher-only image FROM the E1 digest. **[UPDATED 2026-09-14 — B56 §2] BUILT and verified:** `plantseg-teacher:local`, image id `sha256:8c1f31f6…9ba45`, revision label `1dbd1a1`; the build-time assertion printed `MODELS registry entries = 92`, and a runtime re-measurement gave `92`. **Still open: a push to a registry**, which is separately gated — a pod cannot pull a local tag. ~~**[UPDATED 2026-09-20 — build half re-measured, push half STILL OPEN]**~~ *(superseded the same day by the CLOSED note below)* Local image re-verified at `plantseg-teacher:local`, id `sha256:cb413304…c32c2b`, revision label `48bccc70caef…4368f5`, base digest `sha256:b80b645d…866aaf`. **Nothing is published:** the GHCR package is anonymously readable, but the tag `teacher-48bccc70caef…4368f5` returns HTTP 404 and the tag list carries no `teacher-*` entry at all. Two facts for whoever plans the push, neither of them a decision here: `scripts/build_and_push_image.sh` targets `plantseg-thesis:official` only and has no teacher path; and the teacher image's baked `src/` and `scripts/` come from the base's `f77d05d7` because `Dockerfile.teacher` re-copies no source, so N22's clone-at-the-image-commit step is a precondition for the pushed image being usable rather than a convenience. **[CLOSED 2026-09-20 — PUBLISHED AND ANONYMOUSLY RESOLVABLE]** Tag `ghcr.io/ainsleydeluna/plantseg-thesis:teacher-48bccc70caef4b8e252ffd11182c001f0f4368f5` → OCI index `sha256:cb413304e2445e5c8ac3786f7a370f2ed05a07d11843b11687bb9eb23dc32c2b` (`application/vnd.oci.image.index.v1+json`, 856 B). The digest agreed **three independent ways**: the push output, the registry's `docker-content-digest` header, and a SHA-256 recomputed from the fetched index body. Verified through a **fresh anonymous registry token, with no Docker credentials and without touching the credential store** — anonymous tag resolution **PASS**, anonymous digest resolution **PASS**, anonymous config-blob fetch **PASS** (so the pull path, not merely the manifest lookup, is public). Walked index → `linux/amd64` manifest `sha256:6a6a9394…38dc3` → config `sha256:2610823e…7cf1`: platform `linux/amd64` **verified**, `org.opencontainers.image.revision` = `48bccc70caef4b8e252ffd11182c001f0f4368f5` **verified**, `org.opencontainers.image.base.digest` = `sha256:b80b645d…866aaf` **verified**, `CUBLAS_WORKSPACE_CONFIG=:4096:8` **verified**, `PLANTSEG_GIT_COMMIT=48bccc70caef4b8e252ffd11182c001f0f4368f5` **verified**. The index also carries an `unknown/unknown` buildx attestation manifest, as originally built. **No rebuild:** the pushed object is the existing image — same id and same `CreatedAt 2026-09-14 14:52:47` as the local tag — and no Dockerfile or build script was touched or invoked. **`:official` not moved** (remote still resolves to `sha256:b80b645d…866aaf`, the E1 base) and **`:latest` not created** (HTTP 404); the remote tag list gained exactly one entry. **G20 remains OPEN** — publishing an image is not first-forward attestation, and nothing has yet executed on CUDA | B55 §6.2, §9; `Dockerfile.teacher`; `requirements-teacher.lock`; [docs/runpod_environment.md](../docs/runpod_environment.md) "Authoritative TEACHER-stage image" |
| G13 | `scripts/verify_env.py:61-73` tests layer 1 only. Extend the mmseg/mmcv check to a registration-forcing import, and assert `len(MODELS.module_dict) > 0` | B55 §3.1, §9; `scripts/verify_env.py` |
| G14 | Teacher determinism gap — does MMEngine 0.10.7 set `CUBLAS_WORKSPACE_CONFIG`? **[CLOSED 2026-09-14 — B56 §3.5] No.** Its `deterministic=True` sets `cudnn.deterministic` and `cudnn.benchmark=False`, calls `use_deterministic_algorithms(True)` in strict form, and sets no cuBLAS configuration — and no mmengine, mmseg or mmcv source sets it either. The consequences move to G8 (teacher image) and G15. **[UPDATED 2026-09-15 — B58] Verified overwrite chain**, SOURCE-PROVEN at pinned mmengine 0.10.7 (B58 §3, T2): `Runner.from_cfg` passes `randomness` (`runner.py:487`) into `cls(...)` (`:462`); `__init__` runs `setup_env` (`:372`), which applies the teacher config's `env_cfg.cudnn_benchmark=True` (`:666-667`, B56 §3.3); then `set_randomness` (`:376`) calls `set_random_seed(deterministic=True)` in `mmengine/runner/utils.py` (sha256 `2649223c…a5a3`, equal to the wheel's RECORD, B57 E-3), which sets `cudnn.deterministic=True` and `cudnn.benchmark=False` and calls `use_deterministic_algorithms(True)` without `warn_only` (`utils.py:90`); only after that does `_log_env` (`:403`) run, whose `collect_env()` can initialize CUDA. A differing-seed `resume` re-enters `set_randomness` (`:2053-2060`). The `warn_only` reset and its reversal are MEASURED on CPU through direct `set_random_seed` calls (B57 E-6); nothing ran on CUDA. **Selected correction basis:** G8 supplies the variable (PRIMARY), and the selected G18 `TeacherRunner` overrides `set_randomness` to establish the registered policy before CUDA initialization — PLAN, not implemented (B58 §9) | B55 §6.3, §9; B56 §3; B57 E-3, E-6; B58 §3, §9 |
| G15 | **Teacher strict-mode mismatch.** MMEngine enables determinism without `warn_only` (`mmengine/runner/utils.py:90`), and resets a caller's `warn_only=True` to `False`; ch3 §D and contract B6 both specify `warn_only=True`. `[INFERRED]` the teacher **raises** where E1 only warned. Candidate sites: `NMF2D`'s `torch.bmm` with no cuBLAS configuration, the CE loss, and `IoUMetric`'s `torch.histc` at the first validation. Resolve through launch code (G18) or by amending ch3 §D / B6 — governed either way. Confirmation needs CUDA. **[UPDATED 2026-09-15 — B58] OPEN.** The operational consequence stays `[INFERRED]`, for the unmodified MMEngine path. Evidence levels, as ruled 2026-09-14: the complete strict-mode op set is **NOT DETERMINABLE** from artifacts, and enumerating it needs the teacher graph on CUDA; `torch.bmm` in `NMF2D` (first forward; `doc:95-100`) and `torch.histc` in `IoUMetric` (`iou_metric.py:190-198`, `doc:81`, first validation at iteration 10,000) are identified candidates, both `[INFERRED]`, neither confirmed on CUDA; the CE site (mmseg `reduction='none'`, `cross_entropy_loss.py:45-50`) is **UNSUPPORTED** — the pinned wheel docstring (`doc:75`) lists `NLLLoss` on CUDA without qualification — and is to be settled by P-3, not by a source read; the bilinear backward is **EXCLUDED at source** (`functional.py:4014-4018`), with the limit that the decomposition's own backward is shown silent by E1's log, not the teacher's. Of the two routes above, launch code is selected: the G18 `TeacherRunner` establishes `warn_only=True` before CUDA initialization (PLAN). P-2 and P-3 serve this operational-consequence investigation, not the G18 decision | B56 §4; B57 E-5; B58 §9, §14, §18; `docs/IMPLEMENTATION_CONTRACT.md` B6 |
| G16 | **The teacher config cannot load.** `Config.fromfile` classifies it as lazy-import: `import os` (`:42`) precedes `_base_` (`:44`), Python 3.11 freezes `os`, and mmengine's builtin test returns `False` for a frozen module when run from outside the Python prefix. Blocks survey items C and D, the teacher fine-tune, and the E2/E3 teacher load (`src/distill/segnext_teacher.py:228` → `init_model` → `Config.fromfile`). Fix: swap the two lines. In simulation the predicate returns `False` and the config loads. ~~**Plan pending**~~ **[CLOSED 2026-09-20 — landed in `51697a7`]** The two lines are swapped: `_base_` is at `:42` and `import os` at `:44`. Verified through the operational loader, not by reading the file: `scripts/smoke_teacher_config.py` passes **92/92** inside `plantseg-teacher:local`, `config_loads_via_mmengine_config_fromfile` among them | B56 §5; `configs/teacher/segnext_mscan-b_1xb16-adamw-40k_plantseg116-512x512.py:42,44` |
| G17 | `scripts/smoke_teacher_config.py` cannot see G16 or merged-config normalization. It loads the config by `exec` (`:59`), never through mmengine's loader, and `no_syncbn_anywhere` (`:141`) scans only the delta file. A load check must go through `Config.fromfile` and run from the operational working directory, because the misclassification is CWD-dependent. **[CLOSED 2026-09-20 — landed in `51697a7`]** The smoke loads through `Config.fromfile` (`:76`) after `os.chdir(REPO)` (`:95`), asserts `config_loads_via_mmengine_config_fromfile` (`:110`), and separately guards the `_base_`-ahead-of-every-import ordering so a re-introduction of G16 fails here (`:294`) | B56 §5.4–5.5; `scripts/smoke_teacher_config.py:76,95,110,294` |
| G18 *(raised as N24)* | `scripts/launch_teacher_finetune.py` never calls `runner.train()`, so the teacher stage needs launch code. B55 §9 raised it as non-governed. It is **reclassified by the path list**: the deliverable is code under `scripts/**` or `src/**`, the same basis as G9 and G11. `Runner.__init__` resets `warn_only` (B56 §3.4), which bears on G15. **[UPDATED 2026-09-15 — B58]** Traceability: N24 stays reclassified and superseded by G18, with no separate row in §11.2. **ARCHITECTURE SELECTED:** a minimal `TeacherRunner` subclass, constructed through the normal `TeacherRunner.from_cfg(cfg)` path, that overrides only `set_randomness()`; the authoritative config stays `randomness.seed = 42` and `randomness.deterministic = True`. The first call asserts CUDA is not initialized, asserts the cuBLAS variable, reseeds, establishes the flags, re-asserts CUDA is not initialized, and sets the pre-CUDA marker last; later calls require the marker, re-check the cuBLAS value, reseed and re-apply the flags (B58 §9). `CUBLAS_WORKSPACE_CONFIG` is asserted, never created (G8). ~~**Implementation NOT YET AUTHORIZED** (plan-gated).~~ **[UPDATED 2026-09-20 — IMPLEMENTED in `0bb6996`; CPU VERIFICATION PASSED; CUDA STILL PENDING]** `src/training/teacher_runner.py` implements the selected architecture and `scripts/launch_teacher_finetune.py --launch` wires it. `scripts/smoke_teacher_runner.py` exercises the seam through the real `TeacherRunner.from_cfg` path against a minimal in-memory probe config — the B58 §6 "M arm" shape, so `setup_env` and `set_randomness` run in their real order with no dataset, checkpoint, network or CUDA — and passes **48/48** in `plantseg-teacher:local`: contract-B6 policy exact after construction, including `cudnn_benchmark=False` overriding the config's own `env_cfg.cudnn_benchmark=True`; pre-CUDA marker set; `torch.cuda.is_initialized()` false throughout; numpy and python RNG reseeded to a fresh seed 42, with seed 43 as negative control. Fail-closed confirmed on a missing cuBLAS value, a wrong one (`:16:8`) and `deterministic=False`, each both directly and through `from_cfg`, with the marker left unset every time. The repeat-call guard refused a second call after a failed first call, while the real resume re-entry (`runner.py:2060`) restored the policy and returned torch, numpy and python RNG to seed 42. The hook accepted the valid state, recorded the five-state policy plus module provenance to the message hub, attested each point once, and rejected a policy deviation, a non-`TeacherRunner` type and a missing pre-CUDA marker — without latching a failed point, and leaving the deviating record on the forensic trail as designed. **Regression control:** against a scratch copy of the module with `warn_only=True` deleted, the same smoke drops to **29/32** on three independent checks, so the green run is not a false pass. **What it does NOT show:** the effective state at a real CUDA forward — that is G20 | B55 §6.4, §9; B56 §3.4; B58 §9–13; `src/training/teacher_runner.py`; `scripts/smoke_teacher_runner.py` |
| G19 | **mmengine is missing from contract B6's library table.** *[Raised by ruling 2026-09-14; recorded here 2026-09-15 — B58 reconciliation]* **Verified lock provenance:** `mmengine==0.10.7` is pinned at `requirements-runpod.lock:96` — the lock the base image installs (`Dockerfile:59-62`) — and at `requirements.lock:30` and `requirements-runpod.in:31`; `requirements-e1.txt:8-9` excludes it as teacher-only; `grep -n -i "mmengine" docs/IMPLEMENTATION_CONTRACT.md` exits 1. **Fix:** add the row to the table at `:376-393`, citing its locks as the Pillow row does, with source `[requirements-runpod.lock; requirements.lock]`. **Manuscript — settled by ruling 2026-09-14, for the searchable text only:** in the repo copy `docs/reference/ch3.pdf` (sha256 `8a558e82…`), "mmengine" has no hit, spaced or whitespace-free, while the positive controls `mmcv` and `MMSegmentation` both hit, including on the version-pinned environment page (p. 116); that version-pinned stack omits mmengine. **Limits:** image-only text was not adjudicated, and the search transcript is not embedded in B52 or B56–B58. The manuscript edit is deferred — no verbatim FIND string until that sentence is read from source. Needs a plan + go | `docs/IMPLEMENTATION_CONTRACT.md` B6 (`:376-393`); lockfiles as cited; ch3 p. 116 |
| G20 | **Contract B6 ordering versus the teacher launch path.** *[Raised by ruling 2026-09-14; recorded here 2026-09-15 — B58 reconciliation]* B6 requires all four measures "before CUDA init" (`docs/IMPLEMENTATION_CONTRACT.md:302-303`), without Chapter III's "where applicable" (ch3 p. 121; B58 §16); under the authority order, B6 governs. The post-construction `warn_only=True` override considered first lands after the Runner's environment logging can initialize CUDA (`runner.py:403`), and was rejected (B58 §9.5). **Design-level timing conflict RESOLVED** by the selected G18 architecture, which establishes the policy before CUDA initialization (PLAN). **Still OPEN** pending first-forward CUDA attestation under the actual G18 implementation; a development canary does not replace the official run's own first-train and first-validation attestation. **[UPDATED 2026-09-20 — STILL OPEN, and CPU verification does not narrow it]** The G18 implementation has landed and passes CPU verification, but nothing in this project has executed on CUDA. `scripts/smoke_teacher_runner.py` asserts `torch.cuda.is_initialized() is False` at every point and prints that scope limit in its own final line; a CPU smoke cannot stand in for first-forward attestation | B57 E-6; B58 §9, §13, §15–16 |
...
| N22 | Runbook: a RunPod volume mounted at `/workspace` shadows the image's source tree, so a clone at the image commit is always required. **[CLOSED 2026-09-20 — landed in two places, by role]** The hazard is broader than the original wording: shadowing is only half of it, because `Dockerfile.teacher` re-copies no source and the teacher image's baked `src/`/`scripts/` are the E1 base's `f77d05d7…`, which contain no `teacher_runner.py`. Both halves are now recorded. **Invariant** → [docs/runpod_environment.md](../docs/runpod_environment.md) §4b item 3 ("The baked source is both shadowed and stale"), which also states that container identity and runtime-source identity are separate and that the runtime *file hashes*, not the image revision label, attest which bytes execute. **Operator procedure** → [docs/teacher_prep_runbook.md](../docs/teacher_prep_runbook.md) §4a, a ten-step sequence pinning the image by digest and the runtime source to detached `0bb6996`, with the three runtime-critical SHA-256 values and a stop rule for an ambiguous pre-existing checkout. §4's historical pip-install path is marked superseded at its head so an operator is routed to §4a before provisioning | B55 §4, §9; `docs/runpod_environment.md` §4b; `docs/teacher_prep_runbook.md` §4, §4a, §12 |
| N23 | Step-0 registry check (`import mmseg.models`, `len(MODELS.module_dict) > 0`), as defence in depth behind G12's build-time assertion. *Exercised locally 2026-09-14 (B56 §2); not yet in the runbook* | B55 §3.2, §9 |
| N25 | **The G2 probe cannot see G15.** B55 Appendix A seeds through `src.seeds.set_seed`, which sets `warn_only=True` and the cuBLAS configuration, so the probe reproduces neither of G15's first two candidate sites; and it runs no validation, so it cannot reach the third. **[UPDATED 2026-09-15 — AMEND 1]** No G2 peak-memory figure exists yet: the B55 Appendix A probe was authored but never executed, and as written it records only three of the five required determinism states. Any future G2 memory figure is NOT provisioning-grade unless `CUBLAS_WORKSPACE_CONFIG` is recorded at launch before the adjudicated CUDA-init boundary and all five determinism states are recorded before the first training forward and again before validation. `[INFERRED]` From pinned source (`torch/nn/functional.py:4014`), the bilinear `interpolate` reroute is keyed on determinism being enabled, not on `warn_only`. Fold B56 §4.3's confirmation into the probe before the pod. Do not touch the probe until G16 and survey items C and D land | B56 §4.4; B55 Appendix A |
...
> **[UPDATED 2026-09-14 — B56 §3.5]** The reasoning above assumes a caller that runs `set_seed`. The
> teacher stage has no such caller: MMEngine seeds it and never sets the variable. For
> `Dockerfile.teacher` the `ENV` line is therefore not a ride-along but the fix. The teacher image has
> run nothing yet, so rebuilding it moves no attestation. The default above is unchanged for the E1
> image.
>
> **[UPDATED 2026-09-15 — B58]** That `ENV` line is the PRIMARY mechanism, still to ship. The selected
> G18 `TeacherRunner` asserts the value but never creates it (B58 §9.1, §10).
...
> **[UPDATED 2026-09-14]** B55 §3.1 added the layer rule: *when adjudicating "X is available", name the
> capability and test at that layer.* The ledger continues in B55 §3.2 (instance 6) and in
> [B56](b56_teacher_survey.md) §7 (instances 7–9).
